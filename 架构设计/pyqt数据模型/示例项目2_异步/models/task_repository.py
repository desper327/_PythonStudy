...
class TaskRepository(QObject):
    ...
    task_added = pyqtSignal(Task)  # 任务添加信号
    task_removed = pyqtSignal(int, Task)  # 任务删除信号 (索引, 任务对象)
    task_updated = pyqtSignal(int, Task)  # 任务更新信号 (索引, 任务对象)
    tasks_cleared = pyqtSignal()  # 清空所有任务信号
